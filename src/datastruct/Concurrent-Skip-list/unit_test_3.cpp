...
void skiplist_add(size_t start, size_t end)
{
  ...
  if(start == end)
    skiplist.add(numbers_insert[start], to_string(numbers_insert[start]));
...
int main(int argc, char *argv[])
{
  ...
  cout << "All threads call a function which performs insert, delete, search and range with random numbers (1-5000)."
       << endl;
  cout << "This simulates all operations parallelly on the skip list." << endl;

  cout << "\nThe final elements in the skip list will be based on if insert or delete got scheduled and executed first "
          "for any given element.\n"
       << endl;

  cout << "Few of the numbers from the 'delete vector' are deleted from the skip list and few may still exist since "
          "insert got scheduled after delete for these numbers."
       << endl;
  cout << "But the tests notice that none of the numbers not present in the 'delete vector' are deleted. And our "
          "traversal of the skip list is sorted. This verifies our skip list to be accurate. \n"
       << endl;
